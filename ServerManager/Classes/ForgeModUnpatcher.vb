...
Public Class ForgeModUnpatcher
    ...
    End Sub
    Function GetModInfo() As ModInfo
        Try
            Using reader As New IO.StreamReader(pack.GetEntry("mcmod.info").Open())
                Dim jsonObject As JObject = JsonConvert.DeserializeObject(Of JArray)(reader.ReadToEnd)(0)
                Dim name As JValue = ""
                Dim version As JValue = ""
                If jsonObject.TryGetValue("name", name) AndAlso jsonObject.TryGetValue("version", version) Then
                    Return New ModInfo(name, version)
                Else
                    Return New ModInfo(True)
                End If
            End Using
