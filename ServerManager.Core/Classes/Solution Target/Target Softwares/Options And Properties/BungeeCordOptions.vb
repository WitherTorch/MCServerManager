...
Public Class BungeeCordListener
    ...
    <DisplayName("優先伺服器列表")> <Description("伺服器將嘗試把玩家連結至""優先伺服器列表""中設置的伺服器")>
...
Public Class BungeeCordOptions
    ...
    <DisplayName("禁用指令")> <DefaultValue(New String() {})> <Category("伺服器")> <Description("禁用的指令")>
    ...
    Sub New(filepath As String)
        ...
        If IO.File.Exists(filepath) Then
            With Me
                ...
                InputPropertyValue(jsonObject, "inject_commands", .Inject_commands)
                InputPropertyValue(jsonObject, "forge_support", .Forge_support)
                InputPropertyValue(jsonObject, "player_limit", .Player_limit)
                Dim _permissions As New List(Of BungeeCordPermission)
                For Each child As JProperty In GetJsonObject(jsonObject, "permissions").Children
                ...
                InputPropertyValue(jsonObject, "timeout", .Timeout)
                InputPropertyValue(jsonObject, "log_commands", .Log_commands)
                InputPropertyValue(jsonObject, "online_mode", .Online_mode)
                InputPropertyValue(jsonObject, "disabled_commands", .Disabled_commands)
                For Each child As JProperty In GetJsonObject(jsonObject, "servers").Children
                    ...
                    .Servers.Add(server)
                Next
                If jsonObject.ContainsKey("listeners") Then
                    ...
                    Next
                End If
                InputPropertyValue(jsonObject, "ip_forward", .Ip_forward)
                InputPropertyValue(jsonObject, "network_compression_threshold", .Network_compression_threshold)
                InputPropertyValue(jsonObject, "prevent_proxy_connections", .Prevent_proxy_connections)
                For Each child As JProperty In GetJsonObject(jsonObject, "groups").Children
                ...
                Next
                InputPropertyValue(jsonObject, "connection_throttle", .Connection_throttle)
                InputPropertyValue(jsonObject, "stats", ._Stats)
                InputPropertyValue(jsonObject, "connection_throttle_limit", .Connection_throttle_limit)
                InputPropertyValue(jsonObject, "log_pings", .Log_pings)
                .path = filepath
                reader.Close()
            End With
        Else
    ...
    Overrides Sub SaveOption()
        ...
        writer.Close()
    End Sub
    Sub CreateOptionsWithDefaultSetting(path As String)
        Me.path = path
    End Sub
